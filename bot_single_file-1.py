...
def _ensure_installed(pip_name: str, import_name: str | None = None):
    """Если пакет не установлен — ставим его прямо здесь, чтобы бот
    не падал с ModuleNotFoundError из-за того, что requirements.txt
    не был установлен на сервере."""
    import_name = import_name or pip_name
    try:
        __import__(import_name)
    except ImportError:
        print(f"[setup] Пакет '{pip_name}' не найден, устанавливаю...")
        subprocess.check_call([sys.executable, "-m", "pip", "install", pip_name])


_ensure_installed("aiosqlite")
_ensure_installed("aiogram")
